    ...
    for line in lines:
        ...
        # Check if the line contains "849a:" (Vlan 52)
            ...
            if '#' in line:
                ...
                data.append({'porta':name.group(1)})
                data.append({'status':None})
                if vlan != vlan_id:
                    data[-1]['status']='NOK'                    
...
if __name__ == '__main__':
    ...
    for sw in devices:
        ...
        device.open()
        dados = device.cli(['show interfaces switchport','show mac add dynamic', 'show enviroment power', 'show cdp nei'])
